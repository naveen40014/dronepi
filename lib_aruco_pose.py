...
class ArucoSingleTracker():
    def __init__(self,
        ...
        self.parameters  = aruco.DetectorParameters() #_create()
    ...
    def track(self, loop=True, verbose=False, show_video=None):
        ...
        while not self._kill:
            ...
            corners, ids, rejected = detector.detectMarkers(gray) #,self._aruco_dict,self._parameters,self._camera_matrix, self._camera_distortion)
            ...
            if not ids is None and self.id_to_find in ids[0]:
                ...
                #ret = cv.solvePnP(corners, self.marker_size, self._camera_matrix, self._camera_distortion)
...
if __name__ == "__main__":
    ...
    calib_path  = "/home/silver/Pictures/"
    camera_matrix   = np.loadtxt(calib_path+'cameraMatrix.txt', delimiter=',')
    camera_distortion   = np.loadtxt(calib_path+'cameraDistortion.txt', delimiter=',')                                      
    aruco_tracker = ArucoSingleTracker(id_to_find=0, marker_size=10, show_video=True, camera_matrix=camera_matrix, camera_distortion=camera_distortion)
